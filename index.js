var temp = document.querySelector('.time');
 	...
 	var typed;
 	var seconds = 20;
  var spark = new Audio("sounds/spark.wav");
  			...
  			for (var i = 0; i < spans.length; i++) {
  				if (spans[i].innerHTML === typed) { // if typed letter is the one from the word
  					if (spans[i].classList.contains("bg")) { // if it already has class with the bacground color then check the next one
  						continue;
  					} if (spans[i].classList.contains("bg") === false || spans[i+1] === undefined && spans[i+1].classList.contains("bg") !== false ){
              spans[i].classList.add("bg"); //detects similar letter hangman-style
          }
